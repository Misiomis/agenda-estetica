...
class AlarmReceiver : BroadcastReceiver() {
    ...
    companion object {
        private const val TAG = "MimarTAlarm"
        ...
        /** Detiene el audio y la vibración activos (llamado desde AlarmActivity o ACTION_DISMISS). */
        @Synchronized
        fun stopAudio() {
            stopJob?.cancel()
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ...
            ACTION_FIRE, null -> {
                // AlarmManager.cancel() no puede retractar un broadcast que el
                // sistema ya despachó — si una reserva se borra justo cuando su
                // alarma está a punto de sonar, el ACTION_FIRE puede llegar de
                // todos modos. Antes de reproducir nada, se confirma la vigencia
                // contra el último estado sincronizado en Room (test_alarm no
                // vive en Room, así que esas no se descartan por esto).
                val pending = goAsync()
                scope.launch {
                    try {
                        if (isStillValid(context, key)) {
                            handleFire(context, key, pName, svc, box, type)
                        } else {
                            Log.i(TAG, "Alarma '$key' descartada al disparar: ya no está SCHEDULED (reserva borrada/cancelada)")
                        }
                    } finally {
                        pending.finish()
                    }
    ...
    /** true si la key no está en Room (p. ej. test_alarm) o sigue SCHEDULED. */
    private suspend fun isStillValid(ctx: Context, key: String): Boolean {
        val entity = AppDatabase.get(ctx).alarmDao().getByKey(key) ?: return true
        return entity.status == AlarmStatus.SCHEDULED
    }
    ...
    private fun handleFire(ctx: Context, key: String, name: String, svc: String, box: String?, type: String) {
        ...
        if (prefs.alarmMode != "vibration_only") playAlarmSound(ctx, prefs.alarmSoundUri)
        if (prefs.alarmMode != "sound_only") startVibration(ctx)
        stopJob = scope.launch { delay(MAX_RING_MS); stopAudio() }
        showNotification(ctx, key, name, svc, box, type)
        launchAlarmActivity(ctx, key, name, svc, box, type)
    }

    private fun playAlarmSound(ctx: Context, customUriStr: String?) {
        val uri = customUriStr?.let { runCatching { Uri.parse(it) }.getOrNull() }
    ...
    private fun startVibration(ctx: Context) {
        val pattern = longArrayOf(0, 700, 400, 700, 400, 700, 400)
        try {
            ...
            // Silencioso si el dispositivo no tiene vibrador
        }
    }
    ...
    private fun updateStatus(ctx: Context, key: String, status: AlarmStatus) {
        scope.launch { AppDatabase.get(ctx).alarmDao().updateStatus(key, status, System.currentTimeMillis()) }
    }
